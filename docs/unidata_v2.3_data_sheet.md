# Unidata v2.3 — data sheet

**Product label:** “v2.3” refers to the **Unidata snapshot in PostgreSQL** after the Santa Clara parcel GPKG backfill (and any other changes applied outside this file). The database does **not** store a `version = 2.3` column; this document is the release record.

**Record date:** 2026-05-15 (machine local time for file stamps)
...
**Machine-readable missingness (same rows as below):** `docs/unidata_v2.3_field_missingness_step1.csv`  
...
To refresh after a DB change: run `tools/audits/field_missingness_classification.py` (Step 1) or re-run the same `compute_field_rows` + `write_csv` logic and replace `docs/unidata_v2.3_field_missingness_step1.csv`, then update the table in this sheet.

---

## How it was processed (logic)
...
| Field | Value |
|-------|--------|
| **Repository** | `parcelzz/boards` |
| **Branch** | `marwin` |
| **Commit** | **unidata updata(v2.3)** (`fatal: ambiguous argument 'HEAD'`). |
